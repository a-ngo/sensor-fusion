...
void load_calibration_data(cv::Mat &P_rect_00, cv::Mat &R_rect_00,
                           cv::Mat &RT) {
...
void project_lidar_to_camera2() {
  ...
  load_calibration_data(P_rect_00, R_rect_00, RT);

  // TASK project lidar points
  ...
  for (auto it = lidar_points.begin(); it != lidar_points.end(); ++it) {
    ...
    // TASK 2. Then, apply the projection equation as detailed in
    // lesson 5.1 to map X onto the image plane of the camera. Store the result
    // in Y.
    Y = P_rect_00 * R_rect_00 * RT * X;

    // TASK 3. Once this is done, transform Y back into Euclidean
    // coordinates and store the result in the variable pt.
    if (Y.at<double>(2, 0) == 0)
      continue;
    ...
    float max_val = 20.0;
    int red =
        std::min(255, static_cast<int>(255 * abs((val - max_val) / max_val)));
    int green = std::min(
        255, static_cast<int>(255 * (1 - abs((val - max_val) / max_val))));
    cv::circle(overlay, pt, 5, cv::Scalar(0, green, red), -1);
  }

  float opacity = 0.6;
  cv::addWeighted(overlay, opacity, vis_img, 1 - opacity, 0, vis_img);
...
int main() { project_lidar_to_camera2(); }
