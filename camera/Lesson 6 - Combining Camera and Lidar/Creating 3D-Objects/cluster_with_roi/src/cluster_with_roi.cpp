...
void showLidarTopview(std::vector<LidarPoint> &lidarPoints, cv::Size worldSize,
  ...
  for (auto it = lidarPoints.begin(); it != lidarPoints.end(); ++it) {
    ...
    if (zw > -1.40) {
      ...
      int red =
          std::min(255, static_cast<int>(255 * abs((val - maxVal) / maxVal)));
      int green = std::min(
          255, static_cast<int>(255 * (1 - abs((val - maxVal) / maxVal))));
      cv::circle(topviewImg, cv::Point(x, y), 5, cv::Scalar(0, green, red), -1);
    }
  }

  // plot distance markers
  // gap between distance markers
...
void clusterLidarWithROI(std::vector<BoundingBox> &boundingBoxes,
  ...
  cv::Mat P_rect_xx(3, 4, cv::DataType<double>::type);

  // 3x3 rectifying rotation to make image planes co-planar
  cv::Mat R_rect_xx(4, 4, cv::DataType<double>::type);
  ...
  for (auto it1 = lidarPoints.begin(); it1 != lidarPoints.end(); ++it1) {
    ...
    // TASK check wether point has been enclosed by one or by
    // multiple boxes. Accordingly, add Lidar point to bounding box
